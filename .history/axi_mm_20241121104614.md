...
5. **MAIB_REV1DOT1**
    - 若定义该参数且定义了**SL_AIB_GEN1**参数，实例化子模块**dut_slave1**时，调用的是[maib_top][6]进行实例化,同时在实例化该模块时候会include[maib_prog_rev1.1.inc][8]
    - 若该参数未定义且定义了**SL_AIB_GEN2**参数，实例化子模块**dut_slave1**时，
